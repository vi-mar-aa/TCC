...
CREATE PROCEDURE sp_HistoricoEmprestimosDevolvidosCliente -- funcionando
  @email VARCHAR(100)
AS
BEGIN
  ...
  WHERE c.email = @email
    AND e.status_emprestimo = 'devolvido'
...
BEGIN
  ...
  JOIN Cliente c ON c.id_cliente = e.id_cliente
  WHERE c.email = @email
    AND e.status_emprestimo = 'devolvimento'  -- Apenas os devolvidos
...
GO
--exec sp_PostsListar
CREATE PROCEDURE sp_PostsListar -- funcionando lista todos posts
...
CREATE PROCEDURE sp_PostsListar_Publicos -- funcionando, só lista os publicos
...
--exec sp_PostCompleto @id_post = 9
GO
...
GO
--select * from Cliente
--exec sp_LeitorPostsHistorico @email = 'luiggi.alexandre@email.com'
...
GO
-- Pesquisar reservas
/*CREATE PROCEDURE sp_PesquisarReservas
...
BEGIN
  ...
  INSERT INTO Emprestimo (id_cliente, id_funcionario, id_midia, id_reserva, data_emprestimo, data_devolucao, limite_renovacoes, status_emprestimo)
  VALUES (@id_cliente, @id_funcionario, @id_midia, @id_reserva, @data_emprestimo, @data_devolucao, (select limite_emprestimos from Parametros), 'devolvido');
...
END
GO
--exec sp_ExemplarInfoComLeitor @id_midia = 3
...
BEGIN
  SELECT  
    ...
    (SELECT imagem_perfil FROM Cliente WHERE id_cliente = i.id_cliente) AS imagem_cliente
  FROM Indicacao i
...
CREATE PROCEDURE sp_InfosEmprestimosTodos
AS
BEGIN
SELECT 
    CASE 
...
CREATE PROCEDURE sp_InfosEmprestimosAtrasados
AS
BEGIN
SELECT 
    CASE 
...
CREATE PROCEDURE sp_HistoricoEmprestimosPagos
AS
BEGIN
SELECT 
CASE 
...
CREATE PROCEDURE sp_PesquisarEmprestimosAtuaisTodos
    @pesquisa VARCHAR(255)
AS
BEGIN
SELECT 
...
GO
CREATE PROCEDURE sp_SelecionarImagemMidiaPorID
@id_midia INT
AS
BEGIN
...
GO
CREATE PROCEDURE sp_SelecionarImagemClientePorID
@id_cliente INT
AS
BEGIN
